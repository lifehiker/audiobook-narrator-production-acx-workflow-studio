...
- Production database can use `DATABASE_URL`; without it, local SQLite works for development/demo and Docker defaults to `/app/data/prod.db`.
...
- Route smoke tests: `/`, `/pricing`, `/templates`, `/templates/acx-pronunciation-sheet`, `/templates/audiobook-pickup-notes`, `/templates/acx-audition-tracker`, `/blog`, `/login`, and `/signup` returned 200.
